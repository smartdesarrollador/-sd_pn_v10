"""
Area Entity Selector Dialog - Diálogo para seleccionar entidades
...
class AreaEntitySelector(QDialog):
    """Diálogo para seleccionar una entidad para agregar al área"""

    entity_selected = pyqtSignal(str, int, str, list)  # entity_type, entity_id, description, tag_ids

    def __init__(self, entity_type: str, db_manager, area_id: int = None, parent=None):
        """
        Args:
            entity_type: Tipo de entidad ('tag', 'item', 'category', 'list', 'table', 'process')
            db_manager: Instancia de DBManager
            area_id: ID del área (opcional, para auto-agregar items creados)
        ...
        self.db = db_manager
        self.area_id = area_id  # Guardar area_id
    ...
    def init_ui(self):
        ...
        self.description_input.setPlaceholderText("Agrega un comentario o descripción para este elemento en el área...")
        self.description_input.setMaximumHeight(80)
        layout.addWidget(self.description_input)

        # Selector de tags
        tags_label = QLabel("🏷️ Tags:")
        tags_label.setStyleSheet("font-weight: bold; color: #9b59b6;")
        layout.addWidget(tags_label)

        # Importar y crear selector de tags
        from src.core.area_element_tag_manager import AreaElementTagManager
        from src.views.widgets.area_tag_selector import AreaTagSelector

        self.tag_manager = AreaElementTagManager(self.db)
        self.tag_selector = AreaTagSelector(self.tag_manager)
        ...
        # Botón agregar
        self.add_btn = QPushButton("✅ Agregar al Área")
        self.add_btn.setEnabled(False)
        self.add_btn.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
        self.add_btn.setStyleSheet(self._get_button_style("#9b59b6"))
    ...
    def _create_new_item(self):
        ...
        # Conectar señal para auto-agregar al área si existe
        if self.area_id:
    ...
    def _on_new_item_created(self, item_id: int, category_id: int):
        """Agrega automáticamente el item recién creado al área"""
        if not self.area_id:
            return

        try:
            logger.info(f"Auto-adding item {item_id} to area {self.area_id}")

            # Agregar el item al área automáticamente
            success = self.db.add_area_relation(
                area_id=self.area_id,
                entity_type='item',
                entity_id=item_id,
                description='Item creado desde gestor de áreas',
                order_index=None  # Se calculará automáticamente
            )

            if success:
                logger.info(f"Item {item_id} auto-added to area {self.area_id}")
                ...
                logger.error(f"Failed to auto-add item {item_id} to area")

        except Exception as e:
            logger.error(f"Error auto-adding item to area: {e}")
